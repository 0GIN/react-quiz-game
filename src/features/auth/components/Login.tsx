...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (result.success) {
      // Wymuś pełne przeładowanie strony żeby AuthContext się odświeżył
  ...
  return (
    <main className="main">
      <div style={{ gridColumn: 'span 12', display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '70vh' }}>
        <div style={{ maxWidth: '450px', width: '100%' }}>
          <Card className="login-page">
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <h2 style={{ fontSize: '32px', marginBottom: '8px' }}>🎮 Zaloguj się</h2>
              <p className="page-subtitle">Witaj z powrotem w QuizRush!</p>
            </div>

            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', color: '#E0E0E0', marginBottom: '8px', fontWeight: 600, fontSize: '14px' }}>
                  📧 Email
                </label>
                <input
                  type="email"
