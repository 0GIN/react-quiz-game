-- ========================================
-- Aktualizacja statystyk dla Duel
-- ========================================
-- Rozszerza complete_duel_match() o pełne statystyki:
-- - total_wins / total_losses / total_draws
-- - current_streak (passa wygranych)
-- - total_questions_answered
-- - total_correct_answers
-- ========================================

-- Dodaj kolumnę total_draws jeśli nie istnieje
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'total_draws'
    ) THEN
        ALTER TABLE users ADD COLUMN total_draws INTEGER DEFAULT 0;
    END IF;
END $$;
...
BEGIN
    ...
    IF v_winner_id IS NOT NULL THEN
        ...
        WHERE id = v_loser_id;
    ELSE
        -- Remis - obaj dostają po 50 FP, bez resetu streaka, liczony jako remis
        UPDATE users
        SET 
